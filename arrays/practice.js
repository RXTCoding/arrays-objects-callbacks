...
function evenFinder (nums) {
  let evenNum= [] //<-- we need somewhere to store the even numbers that are evaluated as true. Create a variable to store them
  for(let i = 0; i < nums.length; i++){  //<-- we use nums.length b/c the # of runs should not exceed the number on indexes in the vaiable
  nums[i] % 2 === 0 ? evenNum.push(nums[i]): false // <-- We use nums[i] because we want to evaluate one number at a time and not all numbers at once 
  }
  return evenNum //<-- we display only the even numbers we stored in the new evenNum variable 
}
